...
const IpfsUpload: NextPage = () => {
  ...
  const handleIpfsUpload = async () => {
    setLoading(true);
    const notificationId = notification.loading("Cargando IPFS...");
    try {
      const uploadedItem = await addToIPFS(yourJSON);
      notification.remove(notificationId);
      notification.success("IPFS Cargado correctamente");

      setUploadedIpfsPath(uploadedItem.path);
    } catch (error) {
      notification.remove(notificationId);
      notification.error("Error al cargar IPFS");
      console.log(error);
    } finally {
      setLoading(false);
    }
    ...
    <>
      <div className="flex items-center flex-col flex-grow pt-10">
        <h1 className="text-center mb-4">
          <span className="block text-4xl font-bold">Carga de IPFS</span>
        </h1>
