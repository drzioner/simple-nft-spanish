...
const IpfsDownload: NextPage = () => {
  ...
  const handleIpfsDownload = async () => {
    setLoading(true);
    const notificationId = notification.loading("Obtención de datos de IPFS");
    try {
      const metaData = await getMetadataFromIPFS(ipfsPath);
      notification.remove(notificationId);
      notification.success("IPFS descargado correctamente");

      setYourJSON(metaData);
    } catch (error) {
      notification.remove(notificationId);
      notification.error("Error al descargar de IPFS");
      console.log(error);
    } finally {
      setLoading(false);
    }
    ...
    <>
      <div className="flex items-center flex-col flex-grow pt-10">
        <h1 className="text-center mb-4">
          <span className="block text-4xl font-bold">Descarga de IPFS</span>
        </h1>
        ...
        <button
          className={`btn btn-accent my-6 ${loading ? "loading" : ""}`}
          disabled={loading}
